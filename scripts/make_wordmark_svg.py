#!/usr/bin/env python3
"""
Render "RAJAT" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
FONT_PATH = find_font()
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 2 if FONT_PATH and FONT_PATH.endswith(".ttc") else 0))
TEXT = os.environ.get("WORDMARK_TEXT", "RAJAT")

MASK_H = 300           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.12        # extra letter-spacing, in em
LINE_GAP = 1.20
DEPTH_FRAC = 0.34
...
def build_shell():
    ...
    if font is None:
        ...
        l, t, r, b = 0, 0, len(probe) * 20, 30

    h = b - t
    track = int(round(TRACKING * font_size)) if FONT_PATH else 4
    lines = TEXT.split("\n")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">rajat@github: ~$ ./wordmark.sh --3d</text>')
